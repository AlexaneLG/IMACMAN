...
int main(int argc, char** argv) {
    ...
    FilePath texPath = FilePath("../../assets/textures/CloudMap.jpg");
    ...
    File mazeFile;
    //mazeFile = LoadFiles::loadFile("../GLImac-Template/files/grid_laby.txt");
    mazeFile = LoadFiles::loadFile("../../files/grid_laby.txt");

    GameGrid mazeGrid;
    mazeGrid = Import::mazeGridFromFile(mazeFile);
    for(int i =0; i < mazeGrid.nbRows(); i++){
        for(int j = 0; j < mazeGrid.nbCols(); j++){
            std::cout << mazeGrid.elements(i,j);
        }
        std::cout << "\n";
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, cloudTex->getWidth(), cloudTex->getHeight(),0, GL_RGBA, GL_FLOAT, (const GLvoid*)cloudTex->getPixels());
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D,0);
    ...
    //Fill in the VBO to create a box
    glBufferData(GL_ARRAY_BUFFER, box.getVertexCount()*sizeof(ShapeVertex), box.getDataPointer(), GL_STATIC_DRAW); //send vertices data
    ...
    // bind IBO and save it in the VAO
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, box_ibo);
    ...
    TrackballCamera track_camera;
    int current_camera_code = 1;
    //ffly_camera.moveUp(20);
    ffly_camera.moveLeft(-15);
    ffly_camera.moveFront(1.3);
    //ffly_camera.rotateUp(0);

    track_camera.rotateUp(50);
    //track_camera.rotateLeft(0);
    track_camera.moveFront(-50);
    ...
    while(!done) {
        ...
        while(windowManager.pollEvent(e)) {
            ...
            if (windowManager.isKeyPressed(SDLK_c)) switch_cam = true;

            if (current_camera_code == 1) {
                /*mouse events*/
            ...
            if (reset) {
                ...
                //ffly_camera.moveUp(20);
                ffly_camera.moveLeft(-15);
                ffly_camera.moveFront(1.3); //-30
                //ffly_camera.rotateUp(0); //-50
                reset = false;
            }
        }

        if(switch_cam){
           current_camera_code = -current_camera_code;
        }
        ...
        glm::mat4 viewMatrix;
        if(current_camera_code == 1){
            viewMatrix = ffly_camera.getViewMatrix();
        } else{
            viewMatrix = track_camera.getViewMatrix();
        }
        ...
        //Lights
        glm::vec3 lightIntensity = glm::vec3(10,0.55,10);
        ...
        glUniform1i(pacmanProgram.uCloudTexture, 0);
        ...
        //materials
        glUniform3f(pacmanProgram.uKd, 1,0.5,1);
        glUniform3f(pacmanProgram.uKs, 0.5,0.5,1);
        glUniform1f(pacmanProgram.uShininess,3);
        ...
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, cloudTexture);
        glm::mat4 boxMVMatrix = glm::translate(globalMVMatrix, glm::vec3(width, 0, 0));
        boxMVMatrix = glm::scale(boxMVMatrix, glm::vec3(100,100,100));
        boxMVMatrix = glm::translate(boxMVMatrix, glm::vec3(-width, 0, 0));
        glUniform1i(pacmanProgram.uCloudTexture, 0);
        //compute wallMVMatrix and send uniforms
        boxMVMatrix = glm::translate(boxMVMatrix, glm::vec3(width, 0, 0));
        glUniformMatrix4fv(pacmanProgram.uMVMatrix, 1, GL_FALSE,
                           glm::value_ptr(boxMVMatrix));
        glUniformMatrix4fv(pacmanProgram.uMVPMatrix, 1, GL_FALSE,
                           glm::value_ptr(ProjMatrix * boxMVMatrix));
        glUniformMatrix4fv(pacmanProgram.uNormalMatrix, 1, GL_FALSE,
                           glm::value_ptr(glm::transpose(glm::inverse(boxMVMatrix))));
        ...
        glBindVertexArray(box_vao);
        glm::mat4 mazeMVMatrix = glm::translate(globalMVMatrix, glm::vec3(width, 0, 0));//globalMVMatrix;
        for(int i = 0; i < mazeGrid.nbRows(); i++){
            for(int j = 0; j < mazeGrid.nbCols(); j++){
                //compute wallMVMatrix and send uniforms
                mazeMVMatrix = glm::translate(mazeMVMatrix, glm::vec3(width, 0, 0));
                glUniformMatrix4fv(pacmanProgram.uMVMatrix, 1, GL_FALSE,
                                   glm::value_ptr(mazeMVMatrix));
                glUniformMatrix4fv(pacmanProgram.uMVPMatrix, 1, GL_FALSE,
                                   glm::value_ptr(ProjMatrix * mazeMVMatrix));
                glUniformMatrix4fv(pacmanProgram.uNormalMatrix, 1, GL_FALSE,
                                   glm::value_ptr(glm::transpose(glm::inverse(mazeMVMatrix))));


                if(mazeGrid.elements(i,j)==1) {
                    //Draw triangles
            ...
            mazeMVMatrix = glm::translate(mazeMVMatrix, glm::vec3(-width*mazeGrid.nbCols(), 0, 0));
            mazeMVMatrix = glm::translate(mazeMVMatrix, glm::vec3(0, 0, depth));
        }
        glBindVertexArray(0);
        //////////////////////////////////////////////////////////////////////////////////////////////////////
        mazeMVMatrix = globalMVMatrix;
        glBindVertexArray(vao);
        for(int i = 0; i < mazeGrid.nbRows(); i++){
            for(int j = 0; j < mazeGrid.nbCols(); j++){
                //compute wallMVMatrix and send uniforms
                //mazeMVMatrix = glm::scale(mazeMVMatrix, glm::vec3(0.2,0.2, 0.2));
                mazeMVMatrix = glm::translate(mazeMVMatrix, glm::vec3(width, 0, 0));
                /*glUniformMatrix4fv(pacmanProgram.uMVMatrix, 1, GL_FALSE,
                                   glm::value_ptr(mazeMVMatrix));
                glUniformMatrix4fv(pacmanProgram.uMVPMatrix, 1, GL_FALSE,
                ...
                if(mazeGrid.elements(i,j)==2 /*&& cells(i,j).element().eaten() == false*/) {
                    //Draw triangles
                    mazeMVMatrix = glm::scale(mazeMVMatrix, glm::vec3(0.1,0.1, 0.1));
                    glUniformMatrix4fv(pacmanProgram.uMVMatrix, 1, GL_FALSE,
                                       glm::value_ptr(mazeMVMatrix));
                    glUniformMatrix4fv(pacmanProgram.uMVPMatrix, 1, GL_FALSE,
                                       glm::value_ptr(ProjMatrix * mazeMVMatrix));
                    ...
                    //Use glDrawArrays
                    glDrawArrays(GL_TRIANGLES, 0, pacshape.sphere().getVertexCount());
                    mazeMVMatrix = glm::scale(mazeMVMatrix, glm::vec3(10,10, 10));
    ...
    glDeleteVertexArrays(1, &box_ibo);
