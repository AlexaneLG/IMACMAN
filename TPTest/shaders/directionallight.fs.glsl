...
//light
uniform vec3 uLightDir_vs; //vs: working in view space
...
vec3 blinnPhong(vec3 vPosition_vs, vec3 vNormal, vec3 uLightDir_vs, vec3 uLightIntensity, vec3 uKd, vec3 uKs, float uShininess){
	...
	vec3 color = uLightIntensity*(uKd *dot(w_i,vNormal) + uKs*pow(dot(halfVector,vNormal),uShininess));
	return color;
}
void main() {
  ...
  //vec3 vPosition_vs = vPosition * viewMatrix;
	...
	fFragColor = blinnPhong(vPosition_vs, vNormal, uLightDir_vs, uLightIntensity, uKd, uKs, uShininess);
};